...
my_mod <- rand_forest(mtry = tune(),
                      min_n=tune(),
                      trees=100 ) %>% #or 120 or 500 or 1000 consider more trees cause why not?
  set_engine("ranger") %>%
...
my_mod <- rand_forest(mtry = tune(),
                      min_n=tune(),
                      trees=98 ) %>% #or 120 or 500 or 1000 consider more trees cause why not?
  set_engine("ranger") %>%
  set_mode("classification")
## Create a workflow with model & recipe
forest_workflow <- workflow() %>%
...
kaggle_submission <- forest_predictions %>%
  ...
  rename(ACTION=.pred_1)#Just keep datetime and prediction variables
# Write out the file
vroom_write(x=kaggle_submission, file="./forestpreds10.csv", delim=",") 
