...
my_recipe2 <- recipe(type ~ ., data = trainData) %>%
  update_role(id, new_role = "ID") %>%
  step_zv(all_predictors()) %>%
  # if many high-cardinality categorical:
  step_lencode_bayes(all_nominal_predictors(), outcome = vars(type)) %>%
  # maybe interactions (if meaningful) or numeric transforms
  step_mutate_at(all_numeric_predictors(), fn = ~ log1p(.))
...
param_grid2 <- grid_random(
  trees(range = c(500,2000)),
  mtry(range = c(1, floor(sqrt(ncol(trainData)-2)) * 2)),
  min_n(range = c(2,50)),
  size = 30
)

folds2 <- vfold_cv(trainData, v = 5, repeats = 2, strata = type)
...
final_wf2 <- finalize_workflow(forest_wf2, best2)

final_model2 <- fit(final_wf2, data = trainData)

probs_test <- predict(final_model2, new_data = testData, type = "prob")

submission <- testData %>%
  select(id) %>%
  bind_cols(probs_test) # or mutate to correct format for competition

vroom_write(submission, file="./rf_submission.csv", delim=",")
